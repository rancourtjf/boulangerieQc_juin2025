...
$idcommande= htmlspecialchars(trim($data->id_commande));
...
if (isset($idcommande)) {
    $client_id = filter_var($idcommande, FILTER_VALIDATE_INT, [
        'options' => [
            'default' => 'all_client',
            'min_range' => 1
        ]
...
try {
    ...
    $sql = is_numeric($client_id) ? "SELECT * FROM `facture` WHERE id_commande= $idcommande " : "SELECT * FROM `facture`";
