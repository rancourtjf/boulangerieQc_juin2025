...
if (isset($idMaj)) {
    $idMaj = filter_var($idMaj, FILTER_VALIDATE_INT, [
        'options' => [
            'default' => 'all_client',
            'min_range' => 1
        ]
...
try {
    ...
    $sql = is_numeric($idMaj) ? "SELECT * FROM `versionnage` WHERE idMaj='$idMaj'" : "SELECT * FROM `versionnage`";
