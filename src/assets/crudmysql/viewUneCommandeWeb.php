...
if (isset($idcommande)) {
    $nom_commandeCSV = filter_var($nom_commandeCSV, FILTER_VALIDATE_INT, [
        'options' => [
            'default' => 'all_commandeWeb',
            'min_range' => 1
        ]
    ]);
}
...
try {
    ...
    $sql = is_string($nom_commandeCSV) ? "SELECT * FROM `commandeWeb` WHERE nom_commandeCSV= '$nom_commandeCSV' " : "SELECT * FROM `commandeWeb`";
