...
try {
    ...
    $fetch_stmt->bindValue(':idclient', $idclient, PDO::PARAM_INT);
    $fetch_stmt->execute();

    if ($fetch_stmt->rowCount() > 0) :

        $delete_post = "DELETE FROM `client` WHERE idclient=:idclient";
        $delete_post_stmt = $conn->prepare($delete_post);
        $delete_post_stmt->bindValue(':idclient', $idclient,PDO::PARAM_INT);
