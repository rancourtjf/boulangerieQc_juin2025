...
try {
    ...
    $fetch_stmt->bindValue(':id_liste', $id_liste, PDO::PARAM_INT);
    $fetch_stmt->execute();

    if ($fetch_stmt->rowCount() > 0) :

        $delete_post = "DELETE FROM `ingredients` WHERE id_liste =:id_liste";
        $delete_post_stmt = $conn->prepare($delete_post);
        $delete_post_stmt->bindValue(':id_liste', $id_liste,PDO::PARAM_INT);
