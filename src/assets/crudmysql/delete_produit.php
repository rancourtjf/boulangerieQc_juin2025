...
try {
    ...
    $fetch_stmt->bindValue(':id_produit', $id_produit, PDO::PARAM_INT);
    $fetch_stmt->execute();

    if ($fetch_stmt->rowCount() > 0) :

        $delete_post = "DELETE FROM `produits` WHERE id_produit =:id_produit";
        $delete_post_stmt = $conn->prepare($delete_post);
        $delete_post_stmt->bindValue(':id_produit', $id_produit,PDO::PARAM_INT);
