...
$emailUserWeb = $_GET["emailUserWeb"];
$jwt=$_GET["tokenAuth"];
...
if (isset($emailUserWeb)) {
    $courrielID = filter_var($emailUserWeb, FILTER_VALIDATE_INT, [
        'options' => [
            'default' => 'all_client',
            'min_range' => 1
        ]
...
try {
    ...
    $sql = is_string($emailUserWeb) ? "SELECT * FROM `client` WHERE emailUserWeb='$emailUserWeb'" : "SELECT * FROM `client`";
