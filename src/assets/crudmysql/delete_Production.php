...
try {
    ...
    $fetch_post = "SELECT * FROM Production WHERE idProd =:idProd";
    $fetch_stmt = $conn->prepare($fetch_post);
    $fetch_stmt->bindValue(':idProd', $idProd, PDO::PARAM_INT);
    $fetch_stmt->execute();

    if ($fetch_stmt->rowCount() > 0) :

        $delete_post = "DELETE FROM `Production` WHERE idProd =:idProd";
        $delete_post_stmt = $conn->prepare($delete_post);
        $delete_post_stmt->bindValue(':idProd', $idProd,PDO::PARAM_INT);
