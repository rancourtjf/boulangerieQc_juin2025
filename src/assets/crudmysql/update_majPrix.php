...
try {
    ...
    $fetch_post = "SELECT * FROM `majPrix` WHERE dossier=:dossier";
    ...
    if ($fetch_stmt->rowCount() > 0) :
        ...
        $update_query = "UPDATE `majPrix` SET   timestamp=:timestamp WHERE dossier = :dossier";
        $update_stmt = $conn->prepare($update_query);


        $update_stmt->bindValue(':dossier', $data->dossier, PDO::PARAM_INT);
        $update_stmt->bindValue(':timestamp', $data->timestamp, PDO::PARAM_INT);

        if ($update_stmt->execute()) {

            echo json_encode([
                'success' => 1,
                'message' => 'Data mis a jour' . $dossier
            ]);
            exit;
        }
