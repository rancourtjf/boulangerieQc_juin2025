...
if (isset($_GET['idclient'])) {
    $boulanger_id = filter_var($_GET['idclient'], FILTER_VALIDATE_INT, [
        'options' => [
            'default' => 'all_boulanger',
            'min_range' => 1
        ]
    ]);
}
...
try {
    ...
    $sql = is_numeric($boulanger_id) ? "SELECT * FROM `boulanger` WHERE idclient='$boulanger_id'" : "SELECT * FROM `boulanger`";
