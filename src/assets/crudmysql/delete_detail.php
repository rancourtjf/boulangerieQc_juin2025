...
try {
    ...
    $fetch_post = "SELECT * FROM detailproduit WHERE id_detail =:id_detail";
    $fetch_stmt = $conn->prepare($fetch_post);
    $fetch_stmt->bindValue(':id_detail', $id_detail, PDO::PARAM_INT);
    $fetch_stmt->execute();

    if ($fetch_stmt->rowCount() > 0) :

        $delete_post = "DELETE FROM `detailproduit` WHERE id_detail =:id_detail";
        $delete_post_stmt = $conn->prepare($delete_post);
        $delete_post_stmt->bindValue(':id_detail', $id_detail,PDO::PARAM_INT);
